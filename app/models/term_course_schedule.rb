...
class TermCourseSchedule < ActiveRecord::Base
  ...
  def splice_schedules_search_engine
    ...
    tcs = tcs.where("(:start_hour between start_hour AND end_hour) OR (:end_hour between start_hour AND end_hour) OR (start_hour >= :start_hour AND end_hour <= :end_hour)",{:start_hour=>self.start_hour.strftime('%H:%M:%S'),:end_hour=>(self.end_hour-1*60).strftime('%H:%M:%S'),:own_id=>self.term_course_id})
