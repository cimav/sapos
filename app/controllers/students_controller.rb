...
=end
  ...
  def sinodal_certificates
    ...
    Prawn::Document.new(:background => background, :background_scale=>0.36, :margin=>60 ) do |pdf|
      ...
      @parrafo1 = "\nPor medio de la presente tengo el agrado de extender la constancia a <b>#{@asesor}</b> de <b>#{@institution}</b> quien fungio como sinodal externo del examen de Grado presentado el día de hoy, por #{@genero2} alumn#{@genero} <b>#{@nombre}</b> de matrícula <b>#{@matricula}</b> de nuestro programa de <b>#{@programa}</b> con la tesis titulada: <b>#{@thesis_title}</b>\n"
